    ...
    public String getInfo(String student) throws SQLException{
        ...
        try(PreparedStatement st = conn.prepareStatement(
            // replace this with something more useful
                "SELECT (jsonb_build_object('student',idnr,'name',name,'login',login,'program',program,'branch',branch,'finished',(SELECT COALESCE(json_agg(jsonb_build_object('course',courses.name,'code',finishedcourses.course,'grade',finishedcourses.grade,'credits',finishedcourses.credits)),'{}') FROM finishedcourses,courses WHERE finishedcourses.student = ? AND courses.code = finishedcourses.course),'registered',(SELECT COALESCE(json_agg(json_build_object('code',registrations.course,'course',courses.name,'status',status)),'{}') FROM registrations,courses WHERE student = ? AND courses.code = registrations.course),'seminarCourses',seminarcourses,'mathCredits',mathcredits,'researchCredits',researchcredits,'totalCredits',totalcredits,'canGraduate',qualified))AS jsondata FROM basicinformation,pathtograduation WHERE basicinformation.idnr = ? AND pathtograduation.student = ?"
            );){
