...
CREATE FUNCTION try_register() RETURNS trigger AS $$
	...
	END IF;
	-- Check if student already passed course
	IF NEW.course IN (SELECT passedcourses.course FROM passedcourses
			   WHERE student = NEW.student)
	THEN RAISE EXCEPTION 'student has already passed course';
	END IF;
...
CREATE  FUNCTION unregister_from_course() RETURNS TRIGGER AS $$
	...
	BEGIN
		...
		THEN
			...
			THEN --There is room for new student and there is someone in queue
