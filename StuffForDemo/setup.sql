...
--Table to know which departments host which courses.  
...
--Table of all courses that have prerequisites 
CREATE TABLE Prerequisites(
	course CHAR(6) REFERENCES Courses (code),
	prerequisite CHAR(6) REFERENCES Courses (code),
	PRIMARY KEY (course,prerequisite)
);
--Table for all courses that have a limited amount of capacity
CREATE TABLE LimitedCourses(
	code CHAR(6) NOT NULL PRIMARY KEY REFERENCES Courses,
	capacity INTEGER NOT NULL
);
--Table for all students that have chosen a branch.
--FK (branch,program) to see that branch can be chosen from that program
--FK (student,program) to see that student is reading that program
...
--Table for every course classification
CREATE TABLE Classifications(
	name TEXT NOT NULL PRIMARY KEY
);
--Table for all courses that are classified as something
CREATE TABLE Classified(
	course TEXT NOT NULL REFERENCES courses (code),
	classification TEXT NOT NULL REFERENCES classifications (name),
	PRIMARY KEY (course,classification)
);
--Table for all courses that have to be read for each program
CREATE TABLE MandatoryProgram(
	course TEXT NOT NULL REFERENCES Courses (code),
	program TEXT NOT NULL REFERENCES Programs,
	PRIMARY KEY (course,program)
);
--Table for all courses that have to be read for each branch
...
--Table for all courses that are recommended to be read.
...
--Table for all students that are registered to a course
CREATE TABLE Registered(
	student VARCHAR(10) NOT NULL REFERENCES Students (idnr),
	course TEXT NOT NULL REFERENCES Courses (code),
	PRIMARY KEY(student,course)
);
--Table for all courses each student has taken.
CREATE TABLE Taken(
	student VARCHAR(10) NOT NULL REFERENCES Students (idnr),
	course TEXT NOT NULL REFERENCES Courses (code),
...
--Table for students waiting to get registered to a course
...
--View that displays some basic information about students
...
--View that displays all courses finished by each student
CREATE VIEW FinishedCourses AS(
	SELECT student,course,grade,credits FROM taken
	LEFT JOIN courses
	ON courses.code = taken.course
);
--View that displays all courses passed by each student
CREATE VIEW PassedCourses AS(
	SELECT student,course,credits FROM finishedcourses
	WHERE grade != 'U'
);
--View that displays all student registered or waiting to be registered to a course
...
--View that displays all unread mandatory courses for each student
...
--View that displays info regarding each students progress towards graduation
...
--A bunch of insert, Used for testing.
...
INSERT INTO Students VALUES ('4444444444', 'N4', 'ls4', 'Prog2');
...
INSERT INTO Prerequisites VALUES ('CCC888', 'CCC777');
...
INSERT INTO Taken VALUES ('3333333333', 'CCC777', 5);
INSERT INTO Taken VALUES ('3333333333', 'CCC888', 'U');